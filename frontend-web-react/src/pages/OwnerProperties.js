        ...
        const f = async() => {
          ...
          if (session[1] === undefined) {
            ...
            fetch(`${variables.API_GET_OWNER}/${phone}`).then((response)=>{
                ...
                setOwner(data);
                fetch(`${variables.API_PROP}/getByOwner/${data.ownerId}`).then((response)=>{
                    return response.json();
                }).then((data)=>{
                    setProperties(data);
                    console.log(data)
    ...
    return(
        <div>
            ...
            {properties.map((property) => (
                <div key={property.prop_id} className="listItem">
                    <p>{property.prop_address}</p>
                    <p>{property.slots}</p>
                    <button className="btn" onClick={() => deleteProperty(property.prop_id)}>del</button>
                </div>
            ))}
            </div>
        </div>
