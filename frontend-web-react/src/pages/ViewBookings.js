    ...
    const customer_id = user.customerId;
      // console.log(user);
        // if(bookings.length()>0){
            axios.post(`${API_CURR_BOOKING}`, {customerId:customer_id})
            .then((res)=>{
                // if(res.status === 400){
                //     setBookings([]);
                // }
        ...
        return (
          <div className="goals">
            <div className="goal">
              ...
              <button onClick={()=>calculate(data.prop_address, data.prop_id)}>Click to get distance and time to reach</button>
              <br></br>
              {(ways === data.prop_id)? <label>Distance: {distance/1000} km, Time: {time/60} minutes</label>:null}
              <br></br>
              <Link to = {`https://www.google.com/maps/dir/${latitude},${longitude}/${data.prop_address}/`}><b>Get Location Details on Google Map</b></Link>
              <label className="margin-set">Vehicle Registration Number: {data.vehicle_reg_no}</label>
              {/* <br></br> */}
              <button
                className="btn1"
                onClick={() => {
                  handleRemPayment(data.bookingId, data.price);
                }}
              >
